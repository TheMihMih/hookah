...
def homePage(request):
    ...
    context = {"games": games, "products": products}
...
def MakeOrderPage(request, table):
    ...
    if request.method == "POST":
        reservation_duration = strptime(
            request.POST.get("reservation_duration"), "%H:%M"
        )
        ...
        expire_time = reservation_time + timedelta(
            hours=reservation_duration.tm_hour, minutes=reservation_duration.tm_min
        )
        order_exist = (
            OrdersModel.objects.filter(table=table)
            .filter(expired_time__gt=reservation_time)
            .filter(order_date__lt=expire_time)
        )

        if order_exist:
            messages.error(
                request, "К сожалению, на данное время выбранный столик недоступен"
            )
...
def addProductProcess(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                exist_food = MenuModel.objects.get(
